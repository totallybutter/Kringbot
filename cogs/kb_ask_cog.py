...
from utils.ask_utils import categorize_question, load_specified_sheet
...
class AskCog(commands.Cog):
    ...
    @commands.slash_command(name="refresh-ask", description="Recache the responses from online")
    ...
    async def refresh_cache(self, ctx: discord.ApplicationContext, cache_name: str):
        ...
        if time_since_last > REFRESH_ASK_COOLDOWN_SECONDS:
            load_specified_sheet(cache_name.lower(), force=True)
    ...
    @commands.slash_command(description="Ask Kringbot a question.")
    ...
    async def ask(self, ctx, question: str):
        ...
        # Special response
        special_responses = load_specified_sheet("specials")
        if special_responses and question.strip() in special_responses:
            response = special_responses[question.strip()].replace("{user}", display_name)
            await ctx.respond(f"**{display_name} asks**: {question}\n**Kringbot says**: {response}")
            return

        # Role-specific response
        role_substring_rules = load_specified_sheet("role_ask_responses")
        ...
        category_keywords = load_specified_sheet("categories")
        responses_by_category = load_specified_sheet("responses")
    ...
    @commands.slash_command(name="show-ask-cache", description="Display specified `ask` internal cache")
    ...
    async def show_ask_cache(self, ctx: discord.ApplicationContext, cache_name: str):
        cache = load_specified_sheet(cache_name.lower())
